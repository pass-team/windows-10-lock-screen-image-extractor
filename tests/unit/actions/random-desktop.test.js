...
describe('Feature random-desktop', () => {
  ...
  it('Should set new desktop wallpaper', async () => {
    const folder = 'D:/w10-startup-lock-screen-extractor-random-desktop-folder/'
    ...
    await getImages(
      {},
      { path: folder, orientation: ORIENTATION_LANDSCAPE, namePattern: IMAGE_NAME_FORMAT_HASH },
      logger,
    );
    await randomDesktop({},
      { path: folder, orientation: ORIENTATION_LANDSCAPE, namePattern: IMAGE_NAME_FORMAT_HASH },
